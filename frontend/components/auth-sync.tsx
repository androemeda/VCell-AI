...
export function AuthSync() {
  ...
  const inFlightRef = useRef(false);                //prevents duplicate simultaneous sync requests
  const syncedSubRef = useRef<string | null>(null); // stores which user has already been synced
  ...
  useEffect(() => {
    ...
    const syncUser = async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted) { // Avoids logging errors from intentionally cancelled requests.
